...
def archive_inactive_users():
    ...
    # Пороговые даты
    now = datetime.now()
    registration_threshold = now - timedelta(days=30)
    activity_threshold = now - timedelta(days=14)
    ...
    if inactive_user_ids:
        ...
        users_to_archive = list(source_collection.find({"user_id": {"$in": inactive_user_ids}}))
